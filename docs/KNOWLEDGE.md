...
> **Scope:** this document predates the implementation. The trait signatures below were **contracts (specifications)** excerpted from the architecture design document.
...
- **Pitfalls** — risks/open questions from the architecture design directly tied to the module.

> The server consists of **7 modules** (······) + the integration binary `sonicbrew`. The audio core (the processing crates), clock sync , and plugin loader  belong to the audio-toolkit workspace (the `-bsd` crate family), and the server consumes them as path dependencies. Audio asset (sample/IR) loading is handled by audio-toolkit file I/O, so **there is no server-side asset module** (§8.2).

---

## 1. session-store  — distributed session/consensus/topology
...
- **MVP / scope:** Included in the MVP (required). **Not included** in the prototype (the prototype = io+graph+resampler+WebSocket). The prototype runs `demo-route` on a hardcoded graph without session-store ([p11 §8.3](../../notes/p11-mvp-scope-design.md)).
...
## 2. net-rtp-aes67  — RTP/AES67 zero-copy distributed network audio
...
- **Key knowledge:**
  ...
  - **PTP clock integration (`audio-clock-bsd`):** precise clocks are mandatory for RTP timestamp alignment and playback-buffer fill. `audio-clock-bsd` provides PTP clock (t_ns) ↔ audio sample index (s) conversion (PLL/FLL loops) — with AES67 compatibility,  is promoted P2→P1 ([P04 §4](../../docs/p04-distributed-system.html), [p10 §4.3, §11.4 #1](../../notes/p10-architecture-design.md)).
  - **SRTP:** `libsrtp2` (BSD-3) — reuse of the WebRTC security path (DTLS-SRTP) ([P03 §6](../../docs/p03-foundational-libraries.html)).
  - **MVP decision:** out of MVP scope (next phase). The MVP is single-host. netmap RTP/AES67 is adopted when distributed/AES67-compatibility requirements arise ([p11 §3.1](../../notes/p11-mvp-scope-design.md)).

- **Section references:** [P04 §2](../../docs/p04-distributed-system.html) (AES67 distributed audio), [P04 §4](../../docs/p04-distributed-system.html) (PTP/clock domain), [P05 §2](../../docs/p05-freebsd-network-leverage.html) (netmap zero-copy), [P03 §6](../../docs/p03-foundational-libraries.html) (libsrtp2), [p10 §8 , §5.1](../../notes/p10-architecture-design.md).
- **MVP / scope:** out of MVP scope (next phase).
...
## 3. gw-pulse  — PulseAudio protocol-compatible gateway
...
- **Key knowledge:**
  ...
  - **License:** `libpulse` LGPL-2.1 **dynamic-linking obligation**. The gateway is isolated in a separate .so/jail so the Rust-only core's licensing simplicity is preserved ([p10 §8 , §6.3](../../notes/p10-architecture-design.md)).
  - **MVP decision:** out of MVP scope (next phase). The MVP prioritizes browser compatibility (WebSocket). PulseAudio compatibility comes when extending to Linux desktop compatibility ([p11 §3.1](../../notes/p11-mvp-scope-design.md)).
...
## 4. gw-alsa  — ALSA(L) PCM plugin emulation gateway
...
- **Key knowledge:**
  ...
  - **FreeBSD limitation inherited:** on FreeBSD, ALSA **cannot access hardware directly** — it goes through an emulation layer over OSS (`audio/alsa-lib` port).  inherits this limitation as-is ([P01 §3](../../docs/p01-freebsd-audio-ecosystem.html), [p10 §8 M11](../../notes/p10-architecture-design.md)).
  ...
  - **MVP decision:** out of MVP scope (lowest priority).
...
- **MVP / scope:** out of MVP scope (add-on).
...
## 5. gw-browser  — WebAudio/WebSocket/WebRTC browser gateway ★ (core of the shared goal)
...
- **Key knowledge:**
  ...
  - **Dataflow (prototype demo):** WebSocket receive → binary PCM (48k/2ch/f32) → `AudioFrame` conversion → Source node insertion. Reverse direction Sink → WebSocket send (bidirectional) ([p11 §2, §8.3](../../notes/p11-mvp-scope-design.md)).
...
- **MVP / scope:** Included in MVP (required,  WebSocket only) + included in the prototype (its browser entry point). Core module of the shared goal.
...
## 6. control-api  — control plane (gRPC/REST), node-graph manipulation API
...
- **Key knowledge:**
  - **Graph manipulation API:** `list_nodes` / `create_node(spec)` / `link(from, to)` / `load_module(path)` — exposes the SessionStore's topology changes as an API ([p10 §8](../../notes/p10-architecture-design.md), [P04 §5.5](../../docs/p04-distributed-system.html)).
  - **REST first:** the MVP is REST-first (`axum`/`hyper`), with gRPC (`tonic`) optional ([p11 §2](../../notes/p11-mvp-scope-design.md)).
  - **HA:** virtual endpoint via `carp(4)` VIP — clients see a single endpoint, with automatic promotion on failure ([p10 §5.1, §8](../../notes/p10-architecture-design.md), [P04 §6.1](../../docs/p04-distributed-system.html)).
  - **Authorization:** mTLS + Capsicum restriction (static crate, separate jail) ([p10 §8](../../notes/p10-architecture-design.md)).
  - **Module loading:** the MVP is static-link only — dynamic `.so` load/unload depends on `audio-plugin-bsd` (later phase) ([p11 §2, §3.1](../../notes/p11-mvp-scope-design.md)).
  - **MVP decision:** included in MVP (required). But dynamic module loading is static-only.

- **Section references:** [P04 §5.5](../../docs/p04-distributed-system.html) (graph manipulation + control plane), [P04 §6.1](../../docs/p04-distributed-system.html) (carp HA), [p10 §8](../../notes/p10-architecture-design.md).
- **MVP / scope:** Included in the MVP (required). Not included in the prototype (the demo uses the hardcoded `demo-route`, [p11 §8.3](../../notes/p11-mvp-scope-design.md)).
- **Pitfalls:** new module (no direct P0X phase basis) — synthesized from P04 §5.5 graph manipulation + general control-plane practice. A kqueue(2)-based event loop is recommended ([p10 §5.1, §8](../../notes/p10-architecture-design.md)).

---

## 7. monitor  — RT audio metrics/observability
...
- **Key knowledge:**
  ...
  - **Measurement tooling:** `metrics` crate + `tracing` (Rust standard). Prometheus/Grafana integration (`export_prometheus`) ([p10 §8](../../notes/p10-architecture-design.md)).
  - **MVP decision:** out of MVP scope (next phase). The MVP keeps **only minimal metrics** (latency/xrun counts) — measured via custom timestamp logging with `std::time::Instant`. The full  feature set (event loop/Prometheus) comes later ([p11 §3.1, §7c measurement tooling](../../notes/p11-mvp-scope-design.md)).
  - **Observability ↔ control linkage:** The control-API monitoring query endpoints consume these metrics.

- **Section references:** [P05 §1](../../docs/p05-freebsd-network-leverage.html) (kqueue event loop), [P07 §6](../../docs/p07-audio-fundamentals.html) (xrun/latency measurement), [p10 §8](../../notes/p10-architecture-design.md).
- **MVP / scope:** out of MVP scope (next phase). During the MVP, latency is measured with minimal `std::time::Instant` logging.
...
- **Principle:** every gateway and net-rtp, after protocol-specific deserialization, is inserted into the core graph via the **same standard `AudioFrame` interface**. This is sonicbrew's implementation of the PipeWire "every client becomes a graph node" model (P02 §4) ([p10 §6.2](../../notes/p10-architecture-design.md)).
- **Implementation pattern:** gw-*/net-rtp implement audio-toolkit's `AudioNode` trait (`audio-core-bsd`, ) and register as Source/Sink nodes via `Graph::add_node`. For the implemented insertion pattern, see [ARCHITECTURE.md](./ARCHITECTURE.md) §4.
- **`AudioFrame` standard:** `{ channels: u16, sample_rate: u32, samples: Vec<f32> }` — consistent across all node inputs/outputs ([p10 §8](../../notes/p10-architecture-design.md)).
...
- **Sample/recording decode:** FLAC/WAV/PCM files are read from local files and decoded into `AudioFrame`s by `audio-codec-bsd` (, `symphonia`-based multi-format decoder) ([P06 §1, §4](../../docs/p06-audio-codecs.html), [P03 §4](../../docs/p03-foundational-libraries.html)).
- **IR (impulse response) loading:** IRs for convolution reverb are loaded from local files by `audio-dsp-bsd` ( `ConvolverReverb::load_ir`) ([P07 §5](../../docs/p07-audio-fundamentals.html), [p10 §8 M03](../../notes/p10-architecture-design.md)).
...
- **Module isolation:** each sonicbrew module is separated into its own `jail(8)` — component privilege separation ([P04 §6.4](../../docs/p04-distributed-system.html)).
- **Capability sandbox:** per-fd rights via `cap_rights_limit(2)` (Capsicum). Gateway modules use the fd = object-capability model after `cap_enter(2)` — damage is localized when one module is compromised ([P05 §4](../../docs/p05-freebsd-network-leverage.html)).
- **HA VIP:** `carp(4)` (VRRP-equivalent) for  session HA +  control-API VIP ([P04 §6.1](../../docs/p04-distributed-system.html)).
- **NIC redundancy:** multipathing the  RTP/ PTP paths via `lagg(4)` (LACP/active-backup) — preventing audio dropouts ([P04 §6.2](../../docs/p04-distributed-system.html)).
- **MVP limitation:** the MVP is statically linked — jails/Capsicum/carp/lagg are **activated when scaling out**. During the MVP, the only FreeBSD kernel dependency is kqueue ( minimum) ([p11 §5.2 kernel dependency table](../../notes/p11-mvp-scope-design.md)).
...
| Axis | Owner | Essence |
|----|------|------|
| **State consensus** |  session-store | Replicates the topology canon across multiple nodes via Raft leader election + log replication. Session-state persistence (WAL) is handled by session-store itself (`redb`) |
| **Clock sync** | `audio-clock-bsd`  | PTPv2 (IEEE 1588-2008) hardware timestamping + NTP fallback. PTP clock (t_ns) ↔ sample index (s) conversion (PLL/FLL) |
| **Distributed network audio** |  net-rtp-aes67 | AES67 multicast RTP/UDP + netmap zero-copy. Audio packets distributed between nodes |
| **Distributed graph** |  core graph (`audio-core-bsd`) | The topology is treated as a single graph spanning distributed nodes (P04 §5.5) |
...
- **LGPL dynamic linking:** `libpulse` , `libasound`  — separate .so/jail to protect the core's licensing simplicity.
...
- **Distributed graph execution:** distribute independent subtrees of the audio DSP graph across multiple nodes → load-balancing DSP between nodes. The topology canon is kept consistent by  (Raft), and the distributed graph ( core) is treated as a single graph ([P04 §5.5](../../docs/p04-distributed-system.html), §1).
