...
> **Scope:** Pre-code (documentation only). Of the targets below, values confirmed in P10/P11 are marked with citations; unconfirmed numbers are explicitly marked **TBD (benchmark back-calculation)** — arbitrary fabrication is prohibited.
...
|------|------|--------|------|-----------|-----------|----------|
| **xrun rate** | Buffer underrun/overrun count per unit time | **0 (absolute)** | Confirmed (P11 §7a) | Monitor the xrun counter during sustained playback of 1 second or longer | Custom harness xrun counter (`--features rt-safety --test rt_xrun`) | xrun = 0 during 1s+ playback; xrun = 0 over 1000 cycles under load |
...
| **End-to-end latency** | Total time from source input → sink output | Per path (§4 latency budget table) | Confirmed (P11 §7c) | Send timestamp ↔ audible-time deviation | `std::time::Instant` (MVP) / M14 kqueue timestamps (P1) | Local < 5ms / WS one-way < 100ms (§4) |
...
| **RT-thread dynamic allocation** | Count of `alloc` calls inside RT-thread entry points (`process_cycle`/`process_into_buffer`) | **0 (absolute)** | Confirmed (P11 §7a M04 #3) | Count RT-path allocations with `CountingAllocator` | `CountingAllocator` (`--features rt-safety --test rt_alloc_free`) | alloc = 0 across 5 RT-designated nodes × 100 cycles |
...
| **RTP Gbps** | netmap zero-copy throughput of `net-rtp-aes67` (M09) | TBD Gbps | TBD | Measure packets/second processed on netmap rings | netmap statistics + `gstat` | 6.144 Mbps per AES67 48k/L24 multicast stream; verify linear scaling to N streams |
...
| **RT-thread alloc count** | Dynamic allocation on the RT path (restated from §1.1; independent resource metric) | **0** | Confirmed (P11 §7a) | `CountingAllocator` | `CountingAllocator` harness | alloc = 0 (absolute) |
...
| Metric | Definition | Target | Status | Measurement method | Measurement tool | Pass criteria |
|------|------|--------|------|-----------|-----------|----------|
| **SNR** | Signal-to-noise ratio before/after resampler conversion | **≥ 90 dB** | Confirmed (P11 §7a M04 #1-2) | Measure SNR after converting a 440Hz tone between 48k↔44.1k/96k | `--features quality --test snr_threshold -p audio-resample-bsd` + `fixtures/audio/` | 48k↔44.1k SNR ≥ 90dB **and** 48k↔96k SNR ≥ 90dB |
| **PESQ** | Voice-quality score of an Opus encode→decode round trip | **≥ 3.5** | Confirmed (P11 §7a M05 #1) | Compute PESQ after an Opus 48k/2ch round-trip | `--features quality --test opus_pesq -p audio-opus-bsd` | PESQ ≥ 3.5 **or** SNR ≥ 30dB (alternative criterion) |
| **Codec transparency** | Bit accuracy of lossless paths (PCM/WAV) | **bit-exact (byte-exact)** | Confirmed (P11 §7a M06 #2) | Byte-compare original ↔ decoded output | `--features quality --test flac_byte_exact -p audio-codec-bsd` | Sample values byte-identical to the original (lossless) |
| **Codec bitrate accuracy** | Deviation of the actual bitrate after applying Opus `set_bitrate` | Target ±10% | Confirmed (P11 §7a M05 #3) | Measure encoding bitrate after `set_bitrate(64000)` | Custom harness | Actual bitrate ∈ [57 600, 70 400] bps (64000 ±10%) |
| **DSP accuracy** | Amplitude ratio after `GainProcessor` gain application | amplitude × gain ±1% | Confirmed (P11 §7a M03 #2) | Measure input/output amplitude ratio after applying gain 0.5 | Unit test | |output| / |input| ∈ [0.495, 0.505] |

### 1.5 Recovery / Availability

> Failure-recovery targets for the distributed session (`session-store` M07). For distributed-concept background, see [ARCHITECTURE.md](./ARCHITECTURE.md) §7.1 (session consensus axis).
...
> The MVP is single-node (self-leader), so distributed metrics are **measurement targets for the upcoming P1 (multi-node)**. Only the measurement protocols are finalized ([ARCHITECTURE.md](./ARCHITECTURE.md) §7.3 distributed sync visualization).

| Metric | Definition | Target | Status | Measurement method | Measurement tool | Pass criteria |
|------|------|--------|------|-----------|-----------|----------|
| **Scaling efficiency** | Total-throughput growth versus node count (compared to ideal linear) | TBD % | TBD (P1 measurement) | Measure total throughput while scaling nodes 1→2→3→N | Distributed benchmark harness | Throughput / (N × single-node throughput) > TBD % |
| **Inter-node RTP latency** | One-way RTP transfer latency between distributed nodes | < 1 ms | Confirmed (P10 §11.3) | Timestamp deviation from node-A send → node-B receive (PTP-synchronized) | netmap timestamps + PTP (`audio-clock-bsd` M16) | RTP one-way < 1ms (AES67-grade) |
| **Inter-node RTP jitter** | Variation of inter-node RTP latency | < TBD µs | TBD (P1 measurement) | σ of consecutive RTP packet latencies | RTP header timestamp analysis | p99 jitter < TBD µs |
| **Load-balancing efficiency** | Balance of stream distribution across distributed nodes | TBD | TBD (P1 measurement) | Measure the per-node stream-count distribution | Distributed metrics collection | Coefficient of variation (CV) < TBD |

### 1.7 Network

> The `net-rtp-aes67` (M09) netmap zero-copy path plus gateway (`gw-*`) network processing. Mechanisms (netmap/FEC/jitter buffer) are covered in [KNOWLEDGE.md](./KNOWLEDGE.md) §2 (net-rtp).

| Metric | Definition | Target | Status | Measurement method | Measurement tool | Pass criteria |
|------|------|--------|------|-----------|-----------|----------|
| **Packet-loss resilience (FEC)** | No audio-quality degradation within the tolerable packet-loss rate after forward error correction (FEC) is applied | TBD % | TBD (P1 measurement) | Inject deliberate packet drops → FEC recovery → measure SNR | Packet-drop injector + SNR measurement | SNR ≥ 90dB maintained at loss rates ≤ TBD% |
| **Tolerable packet-loss rate** | Maximum packet-loss rate that still holds the quality threshold (SNR ≥ 90dB) | TBD % | TBD (P1 measurement) | Ramp the loss rate up from 0 → detect the SNR threshold crossing | Packet-drop injector + SNR measurement | Threshold loss rate ≥ TBD% |
| **Jitter-buffer depth** | Frames in the receive-side RTP playout buffer | TBD frames | TBD (P1 measurement) | Cross-measure buffer setting × jitter resilience | netmap buffer statistics | depth × 1/rate < latency budget (jitter absorption vs added-latency tradeoff) |
...
| **Gateway throughput** | WebSocket frame throughput of `gw-browser` (M12) | TBD fps | TBD | Measure WS frames/second | criterion (`ws_round_trip`) | Throughput > target client count × frame rate |
...
## 3. Latency Budget Table (promoted from P10 §11.3)
...
|------|----------|--------------------------------------|------|------|
| **Local playback** (M01 OSS via ALSA) | **< 5 ms** | 256-frame buffer @ 48k = 5.3ms (theoretical); measured < 10ms allowed when via-ALSA overhead is tolerated | Confirmed (P11 §7c); measured < 10ms allowed | JACK-grade (P01 §9); M01 output-stream buffer |
| **Distributed RTP** (M09 netmap) | **< 1 ms** (one-way) | netmap zero-copy ring I/O + RTP header processing + PTP timestamp alignment | Confirmed (P10 §11.3); measured in P1 | AES67 standard (P04 §2.1); PTP hardware timestamps (M16) |
| **Browser WebSocket** (M12) | **< 100 ms** (one-way); **< 200 ms** (round-trip) | WS frame deserialization (worker) → rtrb ring → graph RT processing → Sink → WS send | Confirmed (P11 §7c) | WebSocket relaxation of WebRTC 20-100ms (P10 §11.3) (simpler protocol) |
| **Browser WebRTC** (M12) | **20–100 ms** | DTLS-SRTP handshake + Opus RTP encode/decode + network jitter | Confirmed (P10 §11.3); MVP stub | Typical for WebRTC (P08 §1); out of MVP scope per decision #2 |
| **PulseAudio compatibility** (M10) | **50–200 ms** | PulseAudio protocol deserialization + sample-rate mapping + conversion overhead | Confirmed (P10 §11.3); measured in P1 | Typical for PulseAudio (P02 §3) |

> **Note:** The "measured < 10ms allowed" for local playback is an MVP relaxation that tolerates the via-ALSA overhead caused by the absence of a cpal OSS backend (see the note in [p11 §7c](../../notes/p11-mvp-scope-design.md)). Whether to build a direct OSS backend will be decided after P12 benchmarks ([p10 §11.4 #2](../../notes/p10-architecture-design.md)).
...
| Status | Meaning |
|------|------|
| **Confirmed (P10/P11)** | Target verified in the source documents — update the source first when changing |
...
|------|------|--------|------|
| RT/latency | xrun rate | 0 (absolute) | Confirmed (P11 §7a) |
| RT/latency | RT-thread dynamic allocation | 0 (absolute) | Confirmed (P11 §7a M04 #3) |
| RT/latency | End-to-end latency (local) | < 5ms (measured < 10ms allowed) | Confirmed (P11 §7c) |
| RT/latency | End-to-end latency (WS one-way) | < 100ms | Confirmed (P11 §7c) |
...
| Quality | SNR (resampling) | ≥ 90 dB | Confirmed (P11 §7a M04 #1-2) |
| Quality | PESQ (Opus round-trip) | ≥ 3.5 | Confirmed (P11 §7a M05 #1) |
| Quality | Codec transparency (PCM/WAV) | bit-exact | Confirmed (P11 §7a M06 #2) |
| Quality | Bitrate accuracy | ±10% | Confirmed (P11 §7a M05 #3) |
| Quality | DSP gain accuracy | ±1% | Confirmed (P11 §7a M03 #2) |
...
| Distributed/scaling | Inter-node RTP latency | < 1ms | Confirmed (P10 §11.3); measured in P1 |
| Distributed/scaling | Scaling efficiency | TBD % | TBD (P1 measurement) |
| Distributed/scaling | Inter-node RTP jitter | < TBD µs | TBD (P1 measurement) |
| Distributed/scaling | Load-balancing efficiency | TBD | TBD (P1 measurement) |
| Network | FEC resilience (tolerable loss rate) | TBD % | TBD (P1 measurement) |
| Network | Jitter-buffer depth | TBD frames | TBD (P1 measurement) |
...
- [KNOWLEDGE.md](./KNOWLEDGE.md) §8.3 (RT separation principle) · §2 (net-rtp netmap mechanisms) · §7 (M14 metrics tooling) — domain knowledge
